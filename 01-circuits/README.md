# Módulo 1: Circuitos DC e AC

> **Sobre este módulo**: Cada módulo é uma sessão de **1h–3h** onde você primeiro **simula** o circuito, **observa** o comportamento, **questiona** a IA para entender a física, e depois **modela** matematicamente. A matemática vem DEPOIS da intuição.
>
...
>
> **Pré-requisitos**: [Módulo 0, Fases 1-3](../00-math-physics/README.md) (álgebra, trig, complexos, derivadas, integrais). Fases 4-5 para módulos avançados (cálculo vetorial, EDOs).
>
> **Conexões com outros módulos**:
> - **Base de**: [Módulo 0](../00-math-physics/README.md) — álgebra (Mód 0.2), trig/senóides (Mód 0.4), complexos/fasores (Mód 0.5), dB (Mód 0.6), derivadas (Mód 0.7-0.10), integrais (Mód 0.11-0.14), EDOs (Mód 0.18-0.20), matrizes (Mód 0.21-0.22)
> - **Alimenta**: [Módulo 2](../02-electronics/README.md) (Thévenin, polarização, filtros), [Módulo 4](../04-power-industrial/README.md) (trifásico, transformadores), [Módulo 5](../05-control-dsp/README.md) (modelagem de plantas)
> - **Prática real**: [Módulo 6 L.2-L.5](../06-lab/README.md) (multímetro, protoboard, transitórios, osciloscópio)
> - **Segurança**: [HH.1.1](../07-hardware-hacking/README.md) (eletrônica digital), [HH.2.2-2.3](../07-hardware-hacking/README.md) (análise de PCB, identificação de pinos)
>
...
- **Pinos GPIO**: estado ALTO = fonte V_cc com baixo R_th; estado BAIXO = fonte 0V com baixo R_th; estado **Hi-Z** (entrada) = R_th quase infinito
- **Pull-up / Pull-down**: resistores que ancoram sinais em V_cc ou GND para evitar níveis "flutuantes" quando algo entra em Hi-Z
- **Open-Drain**: saída que só puxa para GND. Precisa de um resistor Pull-up externo para nível lógico ALTO. O tempo de subida depende da capacitância parasita (τ = RC)
...
#### Intuição
Um transistor BJT é essencialmente uma **fonte de corrente controlada por corrente**: a corrente do coletor (grande) é β vezes a corrente de base (pequena). Um MOSFET é uma **fonte de corrente controlada por tensão**: I_D = g_m × V_GS. Entender fontes dependentes agora vai tornar o Módulo 2 (Eletrônica) muito mais fácil.
...
- **Bypass / Desacoplamento (Prática HH)**: capacitores (ex: 100nF) cruciais junto a CIs digitais para fornecer picos rápidos de corrente (alta dV/dt) e prevenir "brownouts" (quedas de Vcc)
- **ESR (Resistência Série)**: todo capacitor real possui um resistor parasita que gera calor e ripple de tensão
...
- **Corrente de Saturação (Prática Eng)**: se I exceder o limite magnético (I_sat), o núcleo satura, L despenca para ~0 e ele vira um fio (gera sobrecorrente e fumaça em conversores de potência!)
...
- **Conexão HH e DAC**: Filtros RC passa-baixas são aplicados diariamente para **debounce** (limpar pulos mecânicos de chaves) e para arredondar saídas **PWM** rápidas, convertendo pulsos digitais em tensão contínua (DAC de baixo custo)
...
> **Próximo passo**: [Módulo 2 — Eletrônica Analógica e de Potência](../02-electronics/README.md) — agora que você domina circuitos passivos, é hora de adicionar semicondutores e criar circuitos que amplificam, retificam e convertem energia!
